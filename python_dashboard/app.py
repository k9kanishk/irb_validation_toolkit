...
findings_data = pd.DataFrame({
    ...
    'Module': ['DATA_QUALITY', 'DATA_QUALITY', 'DATA_QUALITY', 'PD', 'PD'],
    'Test': ['Missing Values - LGD', 'Missing Values - Recovery', 'Missing Values - Realized LGD',
             'Overall AUC', 'SME Segment AUC'],
    'Severity': [2, 2, 2, 3, 2],
    'RAG': ['RED', 'RED', 'RED', 'AMBER', 'RED'],
    'Finding': [
        'predicted_lgd 97.9% missing (expected - defaults only)',
        'recovery_months 97.9% missing (expected - defaults only)',
        'realized_lgd 97.9% missing (expected - defaults only)',
        'AUC=0.6176 below 0.70 target',
        'SME segment AUC=0.5920 below minimum threshold'
    ]
})
...
fig_auc.update_layout(title="PD Model Discrimination (AUC) by Segment",
...
fig_calib.update_layout(title="PD Calibration Curve",
...
fig_lgd.update_layout(title="LGD: Predicted vs Realized by Segment (%)",
...
fig_ccf.update_layout(title="CCF: Predicted vs Realized by Segment (%)",
                      barmode='group', template="plotly_white", height=450,
                      yaxis_title="CCF (%)")
...
fig_override.update_layout(title="Override Impact on Default Rates",
...
def kpi_card(title, value, status, color):
    return html.Div([
        html.P(title, style={'color': '#7f8c8d', 'fontSize': '13px',
                             'margin': '0', 'fontWeight': 'bold'}),
        html.H2(value, style={'color': color, 'margin': '5px 0', 'fontSize': '32px'}),
        ...
        'width': '15%', 'display': 'inline-block', 'margin': '0.5%',
        'verticalAlign': 'top'
    })
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.P("10,000 Accounts | 211 Defaults | 2.11% Default Rate | Validation Date: Feb 2026",
    ...
    # Charts Row 1: PD
    html.H2("PD Model Validation",
            style={'padding': '10px 20px', 'color': '#2c3e50', 'borderBottom': '2px solid #3498db'}),
    html.Div([
    ...
    # Charts Row 2: LGD
    html.H2("LGD Model Validation",
            style={'padding': '10px 20px', 'color': '#2c3e50', 'borderBottom': '2px solid #e67e22'}),
    html.Div([
    ...
    # Charts Row 3: CCF + Overrides
    html.H2("EAD/CCF & Override Analysis",
            style={'padding': '10px 20px', 'color': '#2c3e50', 'borderBottom': '2px solid #27ae60'}),
    html.Div([
    ...
    # Findings Table
    html.H2("Validation Findings Log",
            style={'padding': '10px 20px', 'color': '#2c3e50', 'borderBottom': '2px solid #e74c3c'}),
    html.Div([
        dash_table.DataTable(
            ...
            style_data_conditional=[
                {'if': {'filter_query': '{RAG} = "RED"'}, 'backgroundColor': '#fadbd8'},
                {'if': {'filter_query': 
